...
func applyFieldTransformations(valkeyQuery *Query, desc *series.Desc, frame *data.Frame) error {
	if desc.Semantics == "counter" {
		if valkeyQuery.Options.RateConversion {
		...
		if valkeyQuery.Options.TimeUtilizationConversion && valkeyQuery.Format != Heatmap && isTimeUtilization {
